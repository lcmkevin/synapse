...
const server = new Server({ name: "synapse-mcp", version: "0.1.0" }, {
    capabilities: {
        tools: {},
        resources: {},
    },
...
function extractRuleName(content, fallback) {
...
function extractList(content, token) {
    const out = [];
    for (const line of content.split("\n")) {
        const idx = line.indexOf(token);
        if (idx >= 0)
            out.push(line.slice(idx + token.length).trim());
    }
    return out;
}
function cleanRuleBody(content) {
    return content
        .split("\n")
...
function parseSynapseRule(content, filePath) {
...
async function handleCreateRule(args) {
    ...
    const defaultContent = parsed.data.content ||
